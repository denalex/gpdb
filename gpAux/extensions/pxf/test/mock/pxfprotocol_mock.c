...
Datum pxfprotocol_validate_urls(FunctionCallInfo fcinfo) {
	check_expected(fcinfo);
	return (Datum) mock();
}

Datum pxfprotocol_export(FunctionCallInfo fcinfo) {
	check_expected(fcinfo);
	return (Datum) mock();
}

Datum pxfprotocol_import(FunctionCallInfo fcinfo) {
	check_expected(fcinfo);
	return (Datum) mock();
}

static gphadoop_context* create_context(FunctionCallInfo fcinfo, bool is_import) {
...
static void check_caller(FunctionCallInfo fcinfo, const char* func_name) {
	mock();
}
